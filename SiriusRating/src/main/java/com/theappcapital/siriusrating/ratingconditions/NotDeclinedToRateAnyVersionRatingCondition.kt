...
class NotDeclinedToRateAnyVersionRatingCondition(
    private val daysAfterDecliningToPromptUserAgain: UInt,
    private val backOffFactor: Double? = null,
    private val maxRecurringPromptsAfterDeclining: UInt
) : RatingCondition {
    ...
    override fun isSatisfied(dataStore: SiriusRatingDataStore): Boolean {
        ...
        if ((declinedToRateUserActions.count() - 1) > this.maxRecurringPromptsAfterDeclining.toInt()) {
        ...
        val totalDaysAfterUserDeclinedLast = Duration.between(fromDate, nowDate).toDays().toUInt()
        ...
        val totalDaysAfterDecliningToPromptUserAgain = this.calculatedTotalDaysToPromptUserAgain(
            ...
            timesDeclined = declinedToRateUserActions.count().toUInt()
        )
    ...
    fun calculatedTotalDaysToPromptUserAgain(
        ...
        return (daysAfterDecliningToPromptUserAgain.toDouble() * pow(backOffFactor, max(0, timesDeclined.toInt() - 1).toDouble())).toUInt()
    }
