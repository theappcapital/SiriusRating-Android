...
class NotRatedAnyVersionRatingConditionTest {
    ...
    fun test_condition_is_satisfied_when_the_user_did_rate_and_the_days_to_show_prompt_have_passed() {
        ...
        this.inMemorySiriusRatingDataStore.ratedUserActions = listOf(UserAction(appVersion = "0.1-version", date = Instant.now().minus(Duration.ofDays(180))))
    ...
    fun test_condition_is_not_satisfied_because_the_amount_of_days_to_show_the_recurring_prompt_have_not_yet_passed() {
        ...
        this.inMemorySiriusRatingDataStore.ratedUserActions = listOf(UserAction(appVersion = "0.1-version", date = Instant.now().minus(Duration.ofDays(90))))
    ...
    fun test_condition_is_not_satisfied_when_the_max_amount_of_recurring_prompts_was_reached_even_after_the_days_threshold_has_passed() {
        ...
        // Set the user actions where the user already rated 3 times, all long enough ago to satisfy the days threshold.
        this.inMemorySiriusRatingDataStore.ratedUserActions = listOf(
        ...
        // The condition should not be satisfied, because we reached the max amount of recurring prompts (2 after the initial rating),
        // regardless of how much time has passed since the last rating.
        assertFalse(notRatedAnyVersionRatingCondition.isSatisfied(dataStore = inMemorySiriusRatingDataStore))
    }
