...
class NotDeclinedToRateAnyVersionRatingConditionTest {
    ...
    fun test_condition_is_satisfied_when_the_user_did_decline_and_the_days_to_show_prompt_have_passed() {
        ...
        this.inMemorySiriusRatingDataStore.declinedToRateUserActions = listOf(UserAction(appVersion = "0.1-version", date = Instant.now().minus(Duration.ofDays(180))))
    ...
    fun test_condition_is_not_satisfied_because_user_did_decline_but_the_days_to_show_the_recurring_prompt_have_not_yet_passed() {
        ...
        this.inMemorySiriusRatingDataStore.declinedToRateUserActions = listOf(UserAction(appVersion = "0.1-version", date = Instant.now().minus(Duration.ofDays(90))))
    ...
    @Test
    fun test_condition_is_not_satisfied_when_the_max_amount_of_recurring_prompts_was_reached_even_after_the_days_threshold_has_passed() {
        // Create a condition where the user can only be prompted 2 times after it initially got prompted and declined.
        val notDeclinedToRateAnyVersionRatingCondition = NotDeclinedToRateAnyVersionRatingCondition(daysAfterDecliningToPromptUserAgain = 10, maxRecurringPromptsAfterDeclining = 2)

        // Set the user actions where the user already declined 3 times, all long enough ago to satisfy the days threshold.
        this.inMemorySiriusRatingDataStore.declinedToRateUserActions = listOf(
            UserAction(appVersion = "0.1-version", date = Instant.now().minus(Duration.ofDays(365))),
            UserAction(appVersion = "0.2-version", date = Instant.now().minus(Duration.ofDays(365))),
            UserAction(appVersion = "0.3-version", date = Instant.now().minus(Duration.ofDays(365)))
        )

        // The condition should not be satisfied, because we reached the max amount of recurring prompts (2 after the initial decline),
        // regardless of how much time has passed since the last decline.
        assertFalse(notDeclinedToRateAnyVersionRatingCondition.isSatisfied(dataStore = inMemorySiriusRatingDataStore))
    }
